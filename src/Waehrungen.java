...
private static final long CONVPREC  =   10000000L;
private static final long CONVROUND = CONVPREC/2L;

private static final long DEM2EUR   =    5112920L;
private static final long EUR2DEM   =   19558300L;

private static final long ATS2EUR   =     726728L;
private static final long EUR2ATS   =  137603000L;
...
public static long exchange(long valFrom, int wFrom, int wTo) {
	...
	else
	{
		...
		else if (wFrom == EUR)
		{
			...
			{
				return (valFrom * EUR2ATS + sgn(valFrom) * CONVROUND) / CONVPREC;
			}
			else
			{
				return 0L;
		...
		else if (wFrom == ATS)
		{
			if (wTo == EUR)
			{
				return (valFrom * ATS2EUR + sgn(valFrom) * CONVROUND) / CONVPREC;
			}
			else if (wTo == DEM)
			{
				return exchange(exchange(valFrom,wFrom,EUR),EUR,wTo);
